...
function Create()
	this.HomeUID = "FireEngine"..this.Id.u
	this.gatesCount = 1
	this.Speed = -0.2
end
...
function WaitForGateToOpen()
	if this.gatesCount <= myMarker.TotalGates then
		if Object.GetProperty(myMarker,"Authorized"..this.gatesCount) ~= this.HomeUID then
			if Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y <= 6 then
				this.Speed = -0.2
				timePerUpdate = 0
			...
			elseif (Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y) <= 9 then
				if Object.GetProperty(myMarker,"LargeGate"..this.gatesCount) then
					this.Speed = (Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y) - 5.4
				else
					this.Speed = (Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y) - 5.9
				end
				timePerUpdate = 0
			end
		end
...
function CloseGatesBehindMe()
	for i = 1,this.gatesCount do
		if Object.GetProperty(myMarker,"Authorized"..i) == this.HomeUID then
			if this.Pos.y - Object.GetProperty(myMarker,"GatePosY"..i) > 4.5 then
			...
			if this.Pos.y - Object.GetProperty(myMarker,"GatePosY"..i) > 8.5 then 
