		...
		if this.MarkerUID then
			...
			if next(trafficManagers) then
				for n, d in pairs(trafficManagers) do
					local roadToUse = math.random(0,n.GarageCount)
					n.IG = true
					for thatRoadMarker, dist in pairs(roadMarkers) do
						if thatRoadMarker.GarageTraffic then
							if thatRoadMarker.GarageMarkerID == roadToUse then
								MarkerFound = true
								myMarker = thatRoadMarker
...
function WaitForGateToOpen()
	if this.gatesCount <= myMarker.TotalGates then
		if Object.GetProperty(myMarker,"Authorized"..this.gatesCount) ~= this.HomeUID then
			if Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y <= 3 then
				this.Speed = -0.2
				timePerUpdate = 0
			...
			elseif (Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y) < 4.8 then
				if Object.GetProperty(myMarker,"LargeGate"..this.gatesCount) then
					this.Speed = (Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y) - 2.4
				else
					this.Speed = (Object.GetProperty(myMarker,"GatePosY"..this.gatesCount) - this.Pos.y) - 2.9
				end
				timePerUpdate = 0
			end
		end
...
function CloseGatesBehindMe()
	for i = 1,this.gatesCount do
		if Object.GetProperty(myMarker,"Authorized"..i) == this.HomeUID then
			...
			if this.Pos.y - Object.GetProperty(myMarker,"GatePosY"..i) > 3.5 then 
